...
- **Cross-machine, cross-OS, agent-driven**: the same exchange has been run between two physically separate machines — Windows and macOS — with no direct connection between them, coordinating solely through the chain and IPFS, and driven end to end as Model Context Protocol tool calls by an AI agent on each side (one agent's `send_file` to the other's address; the counterpart's `receive_file`, decrypted byte-identical). The protocol's premise — two autonomous agents exchanging a file with no shared account and no central server — is thus demonstrated, not merely asserted.
- **Test discipline**: ~350 unit tests (including adversarial inputs: tampered manifests, wrong-recipient envelopes, malformed keys, commitment mismatches, crafted identity files), 52 contract tests, and a gated integration suite that spins up a local chain, deploys, and runs the full register → claim → send → receive pipeline live.
